...
public class SugestaoRepositoryImpl implements SugestaoRepositoryCustom {
    ...
    public Page<Sugestao> fetchAll(SugestaoDTO filter, Pageable pageable) {
        ...
        if(filter.getTitulo() != null && filter.getTitulo() != "")      query.where(sugestao.titulo.eq(filter.getTitulo()));
        if(filter.getDescricaoSugestao() != null)                       query.where(sugestao.descricaoSugestao.eq(filter.getDescricaoSugestao()));
        if(filter.getProblemaResolvido() != null)                       query.where(sugestao.problemaResolvido.eq(filter.getProblemaResolvido()));
